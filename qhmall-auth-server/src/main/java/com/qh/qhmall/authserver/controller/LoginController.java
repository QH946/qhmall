package com.qh.qhmall.authserver.controller;
...
import com.qh.qhmall.authserver.feign.MemberFeignService;
import com.qh.qhmall.authserver.feign.ThirdPartFeignService;
...
public class LoginController {
    ...
    public R sendCode(@RequestParam("phone") String phone) {
        ...
        String codeNum = String.valueOf(Math.random()).split("\\.")[1].substring(0, 5);
        //拼接验证码
        ...
        System.out.println("手机号：" + phone + " 验证码为：" + codeNum);
        return R.ok();
    }
    ...
    public String register(@Valid UserRegisterVo vos, BindingResult result, RedirectAttributes attributes) {
        ...
        if (result.hasErrors()) {
            ...
            //校验出错回到注册页面
            return "redirect:http://auth.qhmall.com/reg.html";
        }
        ...
        if (!StringUtils.isEmpty(redisCode)) {
            ...
            if (code.equals(redisCode.split("_")[0])) {
                //删除验证码;令牌机制
                stringRedisTemplate.delete(AuthServerConstant.SMS_CODE_CACHE_PREFIX
                        + vos.getPhone());
                //验证码通过，真正注册，调用远程服务进行注册
                R register = memberFeignService.register(vos);
                if (register.getCode() == 0) {
                    //成功
                    return "redirect:http://auth.qhmall.com/login.html";
                } else {
                    //失败
    ...
    public String loginPage(HttpSession session) {
        ...
        if (attribute == null) {
            ...
            return "redirect:http://qhmall.com";
        }
    }
